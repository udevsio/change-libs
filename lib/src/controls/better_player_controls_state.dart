...
abstract class BetterPlayerControlsState<T extends StatefulWidget> extends State<T> {
  ...
  bool isVideoFinished(VideoPlayerValue videoPlayerValue) {
    return videoPlayerValue?.position != null && videoPlayerValue?.duration != null && videoPlayerValue.position >= videoPlayerValue.duration;
  }
  ...
  Widget _buildMoreOptionsListRow(Widget icon, String name, void Function() onTap, Color textColor) {
            ...
            child: Container(
              ...
              child: Stack(
                ...
                Positioned(
                  ...
                  child: Text(
                    text,
                    style: TextStyle(fontWeight: FontWeight.w700, fontSize: 22, color: textColor),
                  ),
                ),
                Container(
  ...
  Widget _buildSpeedRow(double value, Color textColor) {
    ...
    return BetterPlayerMaterialClickableWidget(
      ...
      child: ListTile(
        ...
        trailing: Visibility(
            visible: value == betterPlayerController.videoPlayerController.value.speed,
            child: Icon(
              Icons.done,
              color: textColor,
            )),
  ...
  void _showSubtitlesSelectionWidget() {
    ...
    final noneSubtitlesElementExists = subtitles?.firstWhere((source) => source.type == BetterPlayerSubtitlesSourceType.none, orElse: () => null) != null;
  ...
  Widget _buildSubtitlesSourceRow(BetterPlayerSubtitlesSource subtitlesSource) {
    ...
    final bool isSelected =
        (subtitlesSource == selectedSourceType) || (subtitlesSource.type == BetterPlayerSubtitlesSourceType.none && subtitlesSource?.type == selectedSourceType.type);
  ...
  void _showQualitiesSelectionWidget(String text, Color color, Color textColor) {
    ...
    if (children.isEmpty) {
      children.add(_buildTrackRow(BetterPlayerHlsTrack(0, 0, 0), betterPlayerController.translations.generalDefault, textColor));
    }
  ...
  Widget _buildTrackRow(BetterPlayerHlsTrack track, String preferredName, Color textColor) {
  ...
  Widget _buildAudioTrackRow(BetterPlayerHlsAudioTrack audioTrack, BetterPlayerHlsAudioTrack selectedAudioTrack) {
    assert(audioTrack != null, "Track can't be null");

    final bool isSelected = selectedAudioTrack != null && selectedAudioTrack == audioTrack;
    return BetterPlayerMaterialClickableWidget(
      onTap: () {
        Navigator.of(context).pop();
